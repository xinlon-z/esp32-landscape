...
static const int   kDimTimeoutMs  = 30 * 1000;
static const int   kSleepTimeoutMs = 5 * 60 * 1000;
...
static uint32_t encode(bool ext, int bat, bool dim, bool sleeping)
{
...
void PowerManager::task(void*)
{
    ...
    for (;;) {
        ...
        const uint32_t idle_ms = static_cast<uint32_t>(
            (xTaskGetTickCount() - last_active) * portTICK_PERIOD_MS);
        const IdleMode idle_mode = computeIdleMode(ext, idle_ms);
        const bool should_sleep  = idle_mode == IdleMode::Sleeping;
        const bool should_dim    = should_sleep || idle_mode == IdleMode::Dimmed;

        const uint32_t prev = s_packed.load(std::memory_order_relaxed);
        const uint32_t next = encode(ext, bat_percent, should_dim, should_sleep);
        ...
        if (next != prev) {
            ...
            if (should_dim != prev_dim || should_sleep != prev_sleep) {
...
void PowerManager::init()
{
    ...
    s_packed.store(encode(ext, bat, false, false), std::memory_order_relaxed);
